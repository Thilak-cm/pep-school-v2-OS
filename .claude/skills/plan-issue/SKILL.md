...
- You want to plan work on a GitHub issue from the `Thilak-cm/pep-school-v2-OS` repo
...
3. Present top 20 results with format: `[#123] Issue Title [High] [label1, label2]`
...
- `Bash` (gh CLI) - `gh issue list` to list filtered issues, `gh issue view` to get issue details
...
>
> `/implement-issue` will read the approved plan from context and proceed directly to branch creation, TDD implementation, CI verification, and GitHub sync — no replanning needed.
