/**
 * Seed monthly plan config to Firestore.
 *
 * Writes the instruction prompt and model settings to config/monthly_plan
 * so the prompt test bench can load and iterate on it.
 *
 * Usage:
 *   node scripts/admin/seed-monthly-plan-config.mjs              # dry run
 *   node scripts/admin/seed-monthly-plan-config.mjs --apply      # write to Firestore
 *   node scripts/admin/seed-monthly-plan-config.mjs --verify     # check existing doc
 */
...
const MONTHLY_PLAN_CONFIG = {
  systemPrompt: `You are an expert Montessori primary guide creating a one-month classroom action plan for one child.
...
async function run() {
  const docRef = db.collection("config").doc("monthly_plan");

  if (verify) {
    const snap = await docRef.get();
    if (!snap.exists) {
      console.log("config/monthly_plan does NOT exist");
    } else {
      const data = snap.data();
      console.log("config/monthly_plan EXISTS");
  ...
  console.log("Monthly plan config:");
  console.log(`  model: ${MONTHLY_PLAN_CONFIG.model}`);
  console.log(`  temperature: ${MONTHLY_PLAN_CONFIG.temperature}`);
  console.log(`  max_tokens: ${MONTHLY_PLAN_CONFIG.max_tokens}`);
  console.log(`  systemPrompt: ${MONTHLY_PLAN_CONFIG.systemPrompt.length} chars`);
