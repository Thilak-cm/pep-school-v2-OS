...
describe("Langfuse coverage", () => {
  ...
  for (const file of LLM_CF_FILES) {
    it(`${file} includes LANGFUSE_SECRET_KEY in secrets`, () => {
      ...
      // Every secrets array that binds an LLM API key must also bind Langfuse
      // keys. Arrays without an LLM key (e.g. the #229 Pub/Sub dispatchers,
      // which only publish messages and hold TELEGRAM_BOT_TOKEN) are exempt:
      // they make no LLM calls, so binding tracing secrets there would violate
      // least privilege without adding coverage.
      const secretsArrays = source.match(/secrets:\s*\[[^\]]+\]/g) || [];
      const llmSecretsArrays = secretsArrays.filter(
        (arr) => arr.includes("OPENROUTER_API_KEY") || arr.includes("OPENAI_API_KEY"),
      );
      assert.ok(llmSecretsArrays.length > 0, `No LLM-key secrets arrays found in ${file}`);
      for (const arr of llmSecretsArrays) {
        assert.ok(
