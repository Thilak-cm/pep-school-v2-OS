...
export default function TypingIndicator({ label = 'Coach Pepper is thinking...' }) {
  const visibleLabel = label || 'Coach Pepper is thinking...';
  return (
    <Box sx={{ display: 'flex', justifyContent: 'flex-start', mb: 1, width: '100%', px: 1 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, bgcolor: 'var(--color-neutral-bg)', borderRadius: 4, px: 2, py: 1.5 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.85rem' }}>{visibleLabel}</Typography>
      </Box>
    </Box>
  );
}
