...
function LandingPage({
  ...
  // --- Quick jump pills (role-based) ---
  const quickJumps = [
    { label: 'Stats', icon: <BarChart3 size={18} />, action: () => onNavigate('/stats'), roles: 'all' },
    { label: 'People', icon: <UserPlus size={18} />, action: () => onNavigate('/addUser'), roles: 'admin' },
    { label: 'Export', icon: <Download size={18} />, action: onNavigateToClassroomNotes, roles: 'admin' },
    {
      label: 'Feedback',
      ...
      <Box>
        ...
        <Box sx={{
          ...
          {quickJumps.map((item) => (
            ...
            >
              <Box sx={{ color: 'var(--color-text-soft)', display: 'flex' }}>{item.icon}</Box>
