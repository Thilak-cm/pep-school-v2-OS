...
lesson,Aarav Kumar,2026-01-15,Introduction to Fractions
observation,Priya Sharma,2026-01-16,Worked independently on bead material`;
...
test('parseCSV trims whitespace from all fields', () => {
  const csv = `type,student_name,date,content
  lesson , Aarav Kumar , 2026-01-15 , Intro to Fractions `;
...
test('parseCSV skips empty rows', () => {
  const csv = `type,student_name,date,content
lesson,Aarav,2026-01-15,Fractions

observation,Priya,2026-01-16,Bead work
`;
  const { rows } = parseCSV(csv);
  assert.equal(rows.length, 2);
});
...
test('parseCSV handles quoted fields with commas', () => {
  const csv = `type,student_name,date,content
observation,Aarav Kumar,2026-01-15,"Worked on fractions, multiplication, and division"`;
  const { rows } = parseCSV(csv);
...
test('validateCSV accepts valid rows', () => {
  const rows = [
    { type: 'lesson', student_name: 'Aarav', date: '2026-01-15', content: 'Fractions' },
    { type: 'observation', student_name: 'Priya', date: '2026-01-16', content: 'Bead work' },
  ];
...
test('validateCSV rejects rows with invalid type', () => {
  const rows = [
    { type: 'video', student_name: 'Aarav', date: '2026-01-15', content: 'Something' },
  ];
  const { valid, errors } = validateCSV(rows);
  assert.equal(valid, false);
...
test('validateCSV rejects rows with missing student_name', () => {
  const rows = [
    { type: 'lesson', student_name: '', date: '2026-01-15', content: 'Fractions' },
  ];
  const { valid, errors } = validateCSV(rows);
  assert.equal(valid, false);
...
test('validateCSV rejects rows with missing content', () => {
  const rows = [
    { type: 'observation', student_name: 'Aarav', date: '2026-01-15', content: '' },
  ];
  const { valid, errors } = validateCSV(rows);
  assert.equal(valid, false);
...
test('applyDefaultDate does not modify rows with existing dates', () => {
  const rows = [
    { type: 'lesson', student_name: 'Aarav', date: '2026-02-01', content: 'Algebra' },
  ];
  const result = applyDefaultDate(rows);
