...
export default function DigestWorkbench() {
  ...
  async function handlePromoteConfirm({ fields }) {
    try {
      await promote({ featureId: FEATURE_ID, fields, promptType });
      setPromoteIdx(null);
      setSnackbar({ open: true, message: `Promoted to live config (${promptType} prompt)`, severity: "success" });
    } catch (err) {
