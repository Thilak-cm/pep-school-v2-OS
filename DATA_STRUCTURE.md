...
- `ai_summaries/report_readiness` – on-demand observation quality check (PEP-68). Shape: `{ status: 'ok' | 'no_notes', sentimentScore: number | null, areaBalanceScore: number | null, missingInputFlags: string[], noteCount: number, noteCountAtCheck: number, checkedAt: string (ISO), dateRangeStart: string (ISO), dateRangeEnd: string (ISO), programId: string, model: string, generatedBy: string (userId), generatedByName: string | null }`. Cached per student; staleness tracked via `noteCountAtCheck` vs current observation count. On each recheck, the previous doc (if `status: "ok"`) is archived to `report_readiness/history/{timestamp}` before overwrite (PEP-233).
