...
- `ai_summaries/open_questions` – AI-generated bank of open questions for teacher interviews, organized by exploration area (PEP-173, restructured PEP-207, enriched #144). Generated alongside the soul during monthly regeneration — old doc overwritten in place (no history archival; questions are fully regenerated each month). Archival planned for a future version. Shape: `{ areas: Record<string, Array<{ question: string, status: "pending" | "answered", answeredAt?: Timestamp, method?: "voice" | "manual", observationId?: string, answeredBy?: { uid: string, name: string } }>>, programId: ProgramId, classroomId: string, updatedBy: string, updatedAt: Timestamp }`. Each question is an enriched object with status tracking for the Question Deck UI. The CF writes all questions with `status: "pending"`; teachers update individual question status via the Question Deck (Firestore rules allow classroom-scoped teacher updates). Area names are LLM-generated and unique per student (6-11 areas, 8-10 questions each). Monthly regeneration replaces the entire doc — no cross-version answer carry-forward. Consumed by the Question Deck UI and the AI interview agent (PEP-172/PEP-176/PEP-208).
