...
test("exports bounded recomputeStats logs and classifies OOM failures", async () => {
  const adapter = createCloudLoggingAdapter({
    entries: async (filter) => [[
      { insertId: "oom-1", metadata: { timestamp: "2026-08-12T02:59:00.000Z", severity: "ERROR", labels: { execution_id: "run-1" }, resource: { labels: { function_name: "recomputeStats", region: "asia-south1" } } }, data: "FATAL ERROR: Reached heap limit Allocation failed - JavaScript heap out of memory" },
      { insertId: "event-1", metadata: { timestamp: "2026-08-12T02:58:00.000Z", labels: { execution_id: "run-1" }, resource: { labels: { function_name: "recomputeStats", region: "asia-south1" } } }, data: { jsonPayload: { event: "stats_recompute", classroomCount: 21, observationCount: 42, secret: "filtered" } } },
    ], null],
  });
  const result = await adapter.exportRecomputeStatsLogs({ lookbackMinutes: 5 }, new Date("2026-08-12T03:00:00.000Z"));
  assert.equal(result.logs.length, 2);
  assert.equal(result.counts.oom, 1);
  assert.equal(result.logs[0].category, "oom");
  assert.equal(result.logs[1].jsonPayload.classroomCount, 21);
