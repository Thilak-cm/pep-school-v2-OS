...
import {
  ...
  handleListAlerts,
  handleListJobs,
  handleGetJobExecutions,
  handleGetJobWorkItems,
  handleGetDigest,
  handleListDigestHistory,
  handleExportChatLatencyEvents,
  handleExportStatsRefreshLogs,
...
const HANDLERS = {
  ...
  list_assessments: (p) => handleListAssessments(db, p),
  get_assessment_source: (p) => handleGetAssessmentSource(db, p),
  ...
  list_reports: (p) => handleListReports(db, p),
  ...
  list_plan_feedback: (p) => handleListPlanFeedback(db, p),
  list_testbench_runs: (p) => handleListTestbenchRuns(db, p),
  get_config: (p) => handleGetConfig(db, p),
  list_config: () => handleListConfig(db),
  list_brain: (p) => handleListBrain(db, p),
  get_brain_file: (p) => handleGetBrainFile(db, p),
  list_alerts: (p) => handleListAlerts(db, p),
  list_jobs: () => handleListJobs(db),
  get_job_executions: (p) => handleGetJobExecutions(db, p),
  get_job_work_items: (p) => handleGetJobWorkItems(db, p),
  ...
  export_stats_refresh_logs: (p) => handleExportStatsRefreshLogs(cloudLogging, p),
